...
if ('serviceWorker' in navigator) {
	navigator.serviceWorker.register('sw-gamble.js').then(reg => {
		...
		// If there's a waiting worker, tell it to skipWaiting so it becomes active
		if (reg.waiting) {
		...
		reg.addEventListener('updatefound', () => {
			...
			logRaw('SW update found');
			if (nw) nw.addEventListener('statechange', () => {
				logRaw('SW state: ' + nw.state);
				if (nw.state === 'installed' && navigator.serviceWorker.controller) {
					// New update installed; ask it to activate
					nw.postMessage({type: 'SKIP_WAITING'});
				}
			});
		});

		// When a new worker becomes active, reload so clients use new files
		let refreshing = false;
		navigator.serviceWorker.addEventListener('controllerchange', () => {
			if (refreshing) return;
			refreshing = true;
			logRaw('New service worker activated — reloading page');
...
$('spinBtn').addEventListener('click', async () => {
	...
	// Close overlay button (in case it blocks UI)
	const closeBtn = document.getElementById('closeAppOverlay');
	if (closeBtn) {
			closeBtn.addEventListener('click', ()=>{
					const overlay = document.getElementById('appOverlay');
					if (overlay) overlay.style.display = 'none';
					logRaw('App overlay closed by user');
			});
	}
